...
import {
  ...
  EXTERNAL_PORT,
  HOSTNAME,
  SSP_HOSTS_TO_INTEGRATE,
} from '../../lib/constants.js';
import {InterestGroupHelper} from '../../lib/interest-group-helper.js';
import {getTemplateVariables} from '../../lib/template-utils.js';

/**
 * This is the main DSP router and is responsible for a variety of requests
 * made at the top-level path /dsp. This includes retrieving iframes to include
 * and the interest group to join on advertiser pages.
 * 
 * Path: /dsp/
 */
...
BuyerRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      ads: InterestGroupHelper.getAdsForRequest(req, SSP_HOSTS_TO_INTEGRATE),
    });
  },
);
