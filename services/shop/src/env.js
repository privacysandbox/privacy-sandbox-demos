export const PORT = process.env.PORT || 3000;
export const EXTERNAL_PORT = process.env.EXTERNAL_PORT || 3000;
export const SHOP_HOST = process.env.SHOP_HOST || 'SHOP_HOST';
export const SHOP_DETAIL = process.env.SHOP_DETAIL || 'SHOP_DETAIL';
export const SSP_HOST = process.env.SSP_HOST || 'localhost';
export const SSP_X_HOST = process.env.SSP_X_HOST || 'localhost';
export const SSP_Y_HOST = process.env.SSP_Y_HOST || 'localhost';
export const DSP_HOST = process.env.DSP_HOST || 'localhost';
export const DSP_A_HOST = process.env.DSP_A_HOST || 'localhost';
export const DSP_B_HOST = process.env.DSP_B_HOST || 'localhost';
export const DSP_X_HOST = process.env.DSP_X_HOST || 'localhost';
export const DSP_Y_HOST = process.env.DSP_Y_HOST || 'localhost';
export const AD_SERVER_HOST = process.env.AD_SERVER_HOST || 'localhost';
