...
async function getAdServerAd() {
    const adServerBids = await Promise.all([`${SSP_A_URI}/ad-server-bid`, `${SSP_B_URI}/ad-server-bid`].map(async (dspUrl) => {
        const response = await fetch(dspUrl);
        const result = await response.json();
        return result;
    }));
    const [highestBid] = adServerBids.sort((a, b) => b.adServerAd.bid - a.adServerAd.bid);
