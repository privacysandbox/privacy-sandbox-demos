...
2. Open Chrome DevTools. View Application > Shared Storage - under origin **<https://privacy-sandbox-demos-dsp.dev>** verify that
   `has-reported-content` is set to true ![Shared Storage](./img/reach-measurement-shared-storage1.png)
...
4. [Optional] Click select the report and click "Send Selected Report"
...
6. Reload the page
7. In Chrome DevTools. View Application > Shared Storage - under origin **<https://privacy-sandbox-demos-dsp.dev>** verify that has-reported-content
   is set to true
...
- Convert Content ID to ann aggregation key via `convertContentIdToBucket()` function (line 24).
